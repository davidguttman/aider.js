...
async function setup() {
  ...
  try {
    ...
    console.log(`Syncing Python environment from ${pythonProjectPath}/uv.lock using ${venvPythonPath}...`);
    ...
    console.log(`Explicitly installing dependencies from ${pythonProjectPath} into ${venvPath}...`);
    ...
    if (fs.existsSync(venvPathCleanup)) {
        console.log(`Attempting to clean up potentially incomplete venv at ${venvPathCleanup}...`);
        try {
            fs.rmSync(venvPathCleanup, { recursive: true, force: true });
            console.log(`Cleaned up ${venvPathCleanup}.`);
        } catch (cleanupError) {
            console.error(`Failed to clean up ${venvPathCleanup}: ${cleanupError.message}`);
        }
    }
